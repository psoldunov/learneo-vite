...
function handleSubmit(
	...
	const customReason =
		context.reason.value === 'Intellectual Property Infringement'
...
export default function ReportForm() {
	...
	useEffect(() => {
		if (reason.value === 'Intellectual Property Infringement') {
			if (!is_eu.value) {
			...
			legal_text_ip.updateField({ required: false });
			legal_text_dmca.updateField({ required: false });
			contact_org_name.updateField({
				required: false,
			});
			contact_job_title.updateField({
			...
			{success ? (
				...
				<form
					...
					<FormSection title='Reason for Report'>
						...
						<InputWrap
							title='Please select the reason you believe the content is illegal:'
							type='fieldset'
							required={true}>
							<ReasonGroup
								field_id='reason'
					...
					<FormSection title='Contact Information'>
						{reason.value === 'Illegal or Harmful Speech' ? (
							<NotificationBlock>
								For concerns related to personality rights (e.g., defamatory
								content), your name and email are required for us to process and
